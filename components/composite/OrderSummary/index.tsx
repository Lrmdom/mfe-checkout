...
export const OrderSummary: React.FC<Props> = ({ appCtx, readonly }) => {
  ...
  const isTaxCalculated = appCtx.isShipmentRequired
      ? appCtx.hasBillingAddress && appCtx.hasShippingAddress && appCtx.hasShippingMethod
  ...
  return (
      <div className="w-full max-w-4xl mx-auto p-4" data-testid="order-summary">
        <LineItemsContainer>
          {!readonly && (
              <div className="mb-6">
        ...
        {order?.line_items?.[0]?.metadata?.bookingData && (() => {
          ...
          return (
              <div className="mb-6 p-4 border border-gray-200 rounded-lg bg-white shadow-sm">
                <h3 className="font-bold text-xl mb-4 text-gray-800">{t("orderRecap.bookingDetails", "Detalhes da Reserva")}</h3>

                <div className="space-y-3 text-gray-700 text-sm">
                  <p><strong>Período:</strong> {formatDate(vehicle.start_Date)} até {formatDate(vehicle.end_Date)} ({vehicle.number_of_days} dias)</p>
                  <p><strong>Recolha / Entrega:</strong> {vehicle.pick_location}</p>

                  <div className="flex flex-col gap-1">
                    <p>
                      <strong>Veículo:</strong> {vehicle.vehicleModel} ({vehicle.vehicle_registration_number})
                    </p>
                    {vehicle.modelImage && (
                        <img src={vehicle.modelImage} alt={vehicle.vehicleModel} className="my-2 rounded-md max-w-[120px] h-auto object-cover border border-gray-200" />
                    )}
                    <div className="flex justify-between items-center text-base">
                      <span className="font-medium">Preço base:</span>
                      <span className="font-semibold text-gray-900">{(vehicle.total_amount_cents / 100).toFixed(2)} €</span>
                    </div>
                  </div>
                        ...
                        <ul className="list-none space-y-1 mt-1">
                          {extras.map((extra) => (
                              <li key={extra.id} className="flex justify-between items-center">
                                ...
                                <span className="font-semibold text-gray-900">{(extra.price_amount_cents / 100).toFixed(2)} €</span>
                              </li>
                          ))}
                        </ul>
                      </div>
                        ...
                        <ul className="list-none space-y-1 mt-1">
                          {accessories.map((acc, i) => (
                              <li key={i} className="flex justify-between items-center">
                                <span>{acc.title}</span>
                                <span className="font-semibold text-gray-900">{acc.pricing.toFixed(2)} €</span>
                              </li>
                          ))}
                        </ul>
                      </div>
                  ...
                  {assurance && (
                      <div className="mt-3 pt-3 border-t border-gray-100">
                        <div className="flex justify-between items-center">
                          ...
                          <span className="font-semibold text-gray-900">{assurance.packagePriceBasedOnDays.toFixed(2)} €</span>
                        </div>
                        <span className="block text-gray-600">{assurance.packageDetails.title}</span>
                        <small className="block text-gray-500 mt-1">{assurance.packageDetails.description}</small>
                        <ul className="list-disc list-inside text-xs text-gray-600 mt-2 space-y-0.5">
                          {assurance.packageDetails.includedCoverages.map((cov, i) => (
                              <li key={i}><strong>{cov.title}:</strong> {cov.description}</li>
                          ))}
                        </ul>
                      </div>
                  )}
                ...
                <div className="mt-6 pt-4 border-t border-gray-200 flex justify-between items-center">
                  <span className="text-xl font-bold text-gray-800">Total da reserva:</span>
        ...
        <div className="bg-white p-4 border border-gray-200 rounded-lg shadow-sm space-y-3">
          <CouponOrGiftCard readonly={readonly} setCouponOrGiftCard={appCtx.setCouponOrGiftCard} />
          ...
          <DiscountAmount>{(props) => props.priceCents !== 0 && (
              <div className="flex justify-between items-center text-base text-gray-700">
                <span>{t("orderRecap.discount_amount")}</span>
                <span className="font-semibold text-gray-900">{props.price}</span>
              </div>
          )}</DiscountAmount>
          <AdjustmentAmount>{(props) => props.priceCents !== 0 && (
              <div className="flex justify-between items-center text-base text-gray-700">
                <span>{t("orderRecap.adjustment_amount")}</span>
                <span className="font-semibold text-gray-900">{props.price}</span>
              </div>
          )}</AdjustmentAmount>
          <ShippingAmount>{(props) => appCtx.isShipmentRequired && (
              <div className="flex justify-between items-center text-base text-gray-700">
                <span>{t("orderRecap.shipping_amount")}</span>
                <span className="font-semibold text-gray-900">{!appCtx.hasShippingMethod ? t("orderRecap.notSet") : props.priceCents === 0 ? t("general.free") : props.price}</span>
              </div>
          )}</ShippingAmount>
          <PaymentMethodAmount>{(props) => props.priceCents !== 0 && (
              <div className="flex justify-between items-center text-base text-gray-700">
          ...
          <TaxesAmount>{(props) => (
              <div className="flex justify-between items-center text-base text-gray-700">
            <span>
              <Trans i18nKey={isTaxCalculated && appCtx.taxIncluded ? "orderRecap.tax_included_amount" : "orderRecap.tax_amount"} />
            </span>
                <span className="font-semibold text-gray-900">{isTaxCalculated ? props.price : t("orderRecap.notSet")}</span>
              </div>
          )}</TaxesAmount>
          <GiftCardAmount>{(props) => props.priceCents !== 0 && (
              <div className="flex justify-between items-center text-base text-gray-700">
          ...
          <div className="flex justify-between items-center pt-3 border-t border-gray-200 mt-4">
            <span className="text-xl font-bold text-gray-800">{t("orderRecap.total_amount")}</span>
